...
namespace SanityRewrittenMod.Patches
{
    ...
    internal class PlayerPatcher : BaseUnityPlugin
    {
        ...
        private static bool updatedStamina;
        ...
        private static bool PlayerInsanityPatch()
        {
            if (GameNetworkManager.Instance.gameHasStarted)
            {
                ...
                if (LocalPlayer.isInsideFactory)
                {
                    if (!NearOtherPlayers())
                    {
                        LocalPlayer.insanitySpeedMultiplier = InsanityRemasteredBase.lossWhenNotNearOthers * Mathf.Sqrt(PlayersConnected);
                        LocalPlayer.isPlayerAlone = true;
                        if (InsanityGameManager.Instance.LightsOff)
                        {
                            LocalPlayer.insanitySpeedMultiplier = InsanityRemasteredBase.lossWhenLightsAreOut * Mathf.Sqrt(PlayersConnected);
                        }
                        else if (HallucinationManager.Instance.PanicAttack)
                        {
                            LocalPlayer.insanitySpeedMultiplier = InsanityRemasteredBase.lossWhenPanicking * Mathf.Sqrt(PlayersConnected);
                        }
                        if (FlashlightOn || AdvancedCompanyIntegration.nightVision)
                        {
        ...
        {
            PlayersConnected = StartOfRound.Instance.connectedPlayersAmount;
            PlayersConnected = Mathf.Clamp(PlayersConnected, 1, InsanityRemasteredBase.maxPlayerCountForScalingInsanitySpeed);
